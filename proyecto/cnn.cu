...
__global__ void kernel_convolucion_AoS_MCONST(float *in, float *out, int Mres, int Nres){
	int tid = threadIdx.x + blockDim.x * blockIdx.x;
	if (tid < Mres*Nres){
		int x, y, N_original;
		x = 1 + tid%Nres; //coordenaas del centro de cada sub_matriz
...
void SoA_GPU(){
	...
	Write(hostout, Mres, Nres, "Resultado_SoA_MG.txt\0");

	std::cout << "Tiempo SoA con MG" << ": " << dt << "[ms]" << std::endl;
...
int main(int argc, char **argv){
	...
	AoS_MCOMP_GPU(); // Con Memoria Compartida
